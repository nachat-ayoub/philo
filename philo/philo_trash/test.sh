...
run_test() {
  ...
  if [[ "$status" == "DIE" ]]; then
    ...
    echo -e "${RED}ERROR${NC} (unknown status: $status)"
    ((fail_count++))
  fi
}

if [[ $# -eq 1 && -f "$1" ]]; then
  echo -e "${BLUE}Running custom tests from file: $1...${NC}"
  while IFS= read -r line || [[ -n "$line" ]]; do
    [[ -z "$line" || "$line" =~ ^# ]] && continue # skip empty lines and comments
    run_test $line
  done < "$1"
else
  echo -e "${BLUE}Running default tests for ./philo...${NC}"
